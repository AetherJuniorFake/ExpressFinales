...
router.post('/users/new', function(req, res) {
    ...
    bcrypt.hash(data.password, saltRounds, function (err,   hash) {
        ...
        var json = [];
        var id = 0;
        if (nuevoUser.username == undefined || nuevoUser.password == undefined || nuevoUser.username == "" || nuevoUser.password == "" || nuevoUser.type == "") {
            id++;
            json.push({ "mensaje": "No has llenado todos los datos, intenta de nuevo.", "id": id });
            res.status(500).json({ mensajeError: json, mensajeExito:''});
            ...
            nuevoUser.save(function(error) {
                if (error) {
                    ...
                    if (error.errors.type) {
                        id++;
                        json.push({ "mensaje": "Tipo de usuario no valido", "id": id });
                    }
                    res.status(500).json({ mensajeError: json, mensajeExito:''});
                } else {
